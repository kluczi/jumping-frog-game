...
// draws random car type
void randomCarType(Car &car) {
    int type = RA(0, 2);
    if (type <= 1) { // ~66% chance for enemy car
        car.type = 'E';
...
bool checkNeutralCar(Player &player, Car &car) {
    if (car.type == 'N' &&
        ...
        ((player.pos_y - 1 == car.pos_y && car.direction == 1) || (player.pos_y + 1 == car.pos_y && car.direction == -1))) {
        return true;
    }
    return false;
}
...
bool checkFriendlyCar(Player &player, Car &car) {
    if (!player.attached_to &&
        car.type == 'F' &&
        (player.pos_x - 1 == car.pos_x) &&
        (abs(player.pos_y - car.pos_y) <= 1)) {
        return true;
    }
    return false;
}

// moves all cars and changes their speed on the board based on their type and direction
void moveCars(Board &board, Player &player, Car cars[]) {
    for (int i = 0; i < CARS; i++) {
        if (cars[i].speed_timer == 0) {              // when car is ready to move
            if (!checkNeutralCar(player, cars[i])) { // checks if car isn't blocked by neutral car
                ...
                if (RA(0, 1) == 0) {
                    if (cars[i].pos_y == WIDTH - 1 && cars[i].direction == 1) {
                        ...
                        if (player.attached_to != &cars[i]) { // checks if player isn't attached to friendly car
            ...
            // changes speed according to type
            if (cars[i].type == 'E') {
                if (player.timer % 5 == 0) {
